<?php
        ...
        if($result->num_rows > 0){
            while($row = $result->fetch_assoc()){
                ...
                <div class='row'>
                    ...
                    <div class='col-md-3'>
                        <button type='button' class='btn btn-danger eliminar' id='btnEliminar'><i class='fa fa-trash' aria-hidden='true'></i> Eliminar</button>
                    </div>
                    <div class='col-md-3'>
    ...
    if(isset($_POST) && $_POST['tag']=='eliminar'){
        $id = mysqli_real_escape_string($con,$_POST['id']);
    ...
    if(isset($_POST) && $_POST['tag']=='buscar'){
        ...
        $sql = "SELECT * FROM libros,ubicacion,carrera,tema_general WHERE CONCAT(ID_Libro,Titulo,Autor,Editorial,Carrera_FK,Tema_General_FK) LIKE '%$buscar%' 
        AND Ubicacion_FK=ID_Ubicacion AND Carrera_FK=ID_Carrera AND Tema_General_FK=ID_Tema_Gral";
        $result = $con->query($sql);
        if($result->num_rows > 0){
            while($row = $result->fetch_assoc()){
